...
export const WithdrawModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  const isStarter = highestPlanName.includes('Starter');
  const canWithdraw = !isStarter && mainBalance > 0 && accounts.length > 0;
  const withdrawDisabledReason = isStarter ? "Upgrade required" : (mainBalance <= 0 ? "Insufficient balance" : (accounts.length === 0 ? "No withdrawal accounts" : ""));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (withdrawAmount > mainBalance) {
      showError('Insufficient Balance', `You cannot withdraw more than your available balance of $${mainBalance.toFixed(2)}.`);
      return;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-lg p-0 overflow-hidden bg-white dark:bg-slate-900 border-none shadow-2xl rounded-3xl" hideCloseButton>
        <div className="p-6 sm:p-8 max-h-[85vh] overflow-y-auto custom-scrollbar">
          {/* Modal Header */}
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-bold text-primary dark:text-white">Withdraw Funds</h2>
              <p className="text-sm text-slate-500 mt-1">Move capital to your verified account.</p>
            </div>
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 hover:text-error transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4 space-y-3">
            <button 
              ...
              className="w-full py-4 bg-primary text-white rounded-full font-bold shadow-lg shadow-primary/30 active:scale-[0.98] transition-all duration-150 disabled:opacity-70 disabled:shadow-none"
            >
